...
if ( ! class_exists( 'Moove_UAT_Updater' ) ) {
	class Moove_UAT_Updater {
		...
		public function __construct() {
			...
			elseif ( ( $pagenow === 'admin.php' && isset( $_GET['page'] ) && $_GET['page'] === 'moove-activity' ) ) :
				self::uat_check_for_updates();
			endif;
			// add_action( 'admin_init', array( &$this, 'uat_check_for_updates' ) );
		...
		function uat_update_message_content( $plugin_data, $response ) {
		...
		function uat_plugin_updater_notice() {
			update_option( 'uat_last_checked', strtotime( 'yesterday' ) );
			delete_site_transient('update_plugins');
		}

		function uat_check_for_updates() {
			...
			if ( ( $now - $last_checked ) > $check_interval ) :
				...
				$option_key           = $uat_default_content->moove_uat_get_key_name();
				$uat_key             	= function_exists( 'get_site_option' ) ? get_site_option( $option_key ) : get_option( $option_key );
				$license_key          = sanitize_text_field( $uat_key['key'] );
		...
		function get_addon_updates() {
			...
			if ( $license_key ) :
				...
				foreach ( $plugins as $slug => $info ) :
					if ( isset( $info['TextDomain'] ) && $info['TextDomain'] === 'user-activity-tracking-and-log-addon' ) :
		...
		public function moove_hide_update_notice() {
			...
			if ( $version ) :
				...
				update_option( 'uat_hide_update_notice_' . $user_id, $version );      
			endif;
		}
