...
include "/dbOperations.php";
  ...
  function start_session($id){
    ...
    if(!session_id()){
      session_start();
      insertToSessionTable($wpdb, session_id());
    }
  }
  ...
  //  $wpdb->update("{$wpdb->prefix}session", array("login_attempt"=>(int)$login_attempt + 1, "user_id" => $id), array("session_id"=>$session_id));
    $wpdb->insert("{$wpdb->prefix}session", array("session_id" => $session_id, "user_id" => $id, "login_attempt"=>(int)$login_attempt + 1, "attempt_date" => date('Y-m-d H:i:s')));
  }
  ...
  function addUserToSessionSuccLogin($username){
    ...
    $session_id = session_id();
    $push_array = array("user_id" => $user_id, "session_id" => $session_id, "login_attempt" => 0, "attempt_date" => date('Y-m-d H:i:s'));
